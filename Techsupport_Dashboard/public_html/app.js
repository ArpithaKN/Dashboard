...
//         });
    ...
    var refresh = setInterval(gettransaction, 15000); //setInterval for 30 sec
...
//        stationname = data;
        ...
        $http({
            ...
            if($scope.socketdata[0].status === "Offline"){
                sweetAlert($scope.socketdata[0].status);
            }
        });
...
$scope.socketUnlock = function () {                                       //Unlock the ports and create the checkouts records 
        if ($scope.unit)
            unitstatus = 1;
        else
            unitstatus = 0;
...
//                alert(JSON.stringify(data));
                $http({//to unlock                          
                    method: "POST",
                    url: ipURL80 + "transactions/unlock/socket/techsup",
                    data: data,
                    headers: {
                        "Content-Type": "application/json"
                    }
